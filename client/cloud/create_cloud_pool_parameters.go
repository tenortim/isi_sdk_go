// Code generated by go-swagger; DO NOT EDIT.

package cloud

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"

	strfmt "github.com/go-openapi/strfmt"

	models "github.com/tenortim/isi_sdk_go/models"
)

// NewCreateCloudPoolParams creates a new CreateCloudPoolParams object
// with the default values initialized.
func NewCreateCloudPoolParams() *CreateCloudPoolParams {
	var ()
	return &CreateCloudPoolParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewCreateCloudPoolParamsWithTimeout creates a new CreateCloudPoolParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewCreateCloudPoolParamsWithTimeout(timeout time.Duration) *CreateCloudPoolParams {
	var ()
	return &CreateCloudPoolParams{

		timeout: timeout,
	}
}

// NewCreateCloudPoolParamsWithContext creates a new CreateCloudPoolParams object
// with the default values initialized, and the ability to set a context for a request
func NewCreateCloudPoolParamsWithContext(ctx context.Context) *CreateCloudPoolParams {
	var ()
	return &CreateCloudPoolParams{

		Context: ctx,
	}
}

// NewCreateCloudPoolParamsWithHTTPClient creates a new CreateCloudPoolParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewCreateCloudPoolParamsWithHTTPClient(client *http.Client) *CreateCloudPoolParams {
	var ()
	return &CreateCloudPoolParams{
		HTTPClient: client,
	}
}

/*CreateCloudPoolParams contains all the parameters to send to the API endpoint
for the create cloud pool operation typically these are written to a http.Request
*/
type CreateCloudPoolParams struct {

	/*CloudPool*/
	CloudPool *models.CloudPoolCreateParams

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the create cloud pool params
func (o *CreateCloudPoolParams) WithTimeout(timeout time.Duration) *CreateCloudPoolParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the create cloud pool params
func (o *CreateCloudPoolParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the create cloud pool params
func (o *CreateCloudPoolParams) WithContext(ctx context.Context) *CreateCloudPoolParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the create cloud pool params
func (o *CreateCloudPoolParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the create cloud pool params
func (o *CreateCloudPoolParams) WithHTTPClient(client *http.Client) *CreateCloudPoolParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the create cloud pool params
func (o *CreateCloudPoolParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithCloudPool adds the cloudPool to the create cloud pool params
func (o *CreateCloudPoolParams) WithCloudPool(cloudPool *models.CloudPoolCreateParams) *CreateCloudPoolParams {
	o.SetCloudPool(cloudPool)
	return o
}

// SetCloudPool adds the cloudPool to the create cloud pool params
func (o *CreateCloudPoolParams) SetCloudPool(cloudPool *models.CloudPoolCreateParams) {
	o.CloudPool = cloudPool
}

// WriteToRequest writes these params to a swagger request
func (o *CreateCloudPoolParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if o.CloudPool != nil {
		if err := r.SetBodyParam(o.CloudPool); err != nil {
			return err
		}
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
